...
struct Buffer final
{
    ...
    constexpr Buffer& operator<<(const T value)
    {
        ...
        mSize += snprintf(mBuffer + mSize, Info::len, Info::fmt, detail::native(value));
        return *this;
    }
    ...
    constexpr Buffer& operator<<(detail::Manipulator<T>&& m)
    {
        ...
        mSize += snprintf(mBuffer + mSize, requiredSize, fmt, m.padding, m.value);
    ...
    constexpr Buffer& operator<<(detail::Manipulator<T>&& m)
    {
        ...
        if (m.padding > 0 and absPadding > sv.size())
        {
            for (size_type i = 0; i < absPadding - sv.size(); mBuffer[mSize++] = ' ', ++i);
        }
...
constexpr Buffer& operator<<(Buffer& buf, detail::Manipulator<T>&& m)
{
    Buffer tmp;
    tmp << m.value;
